...
import { Plus, Megaphone, MessagesSquare, X } from "lucide-react";
    ...
    <>
      ...
      <dialog ref={ref} className="modal" style={{ width: `min(${width}px, 94vw)` }} onClick={(e) => { if (e.target === ref.current) ref.current?.close(); }}>
        <div className="modal-in">
          <div className="modal-h"><h3>{title}</h3><button type="button" className="btn sm ghost" onClick={() => ref.current?.close()} aria-label="Закрити"><X size={15} /></button></div>
          {children}
        </div>
      </dialog>
    </>
...
export function ConnectChatDialog({ chats, botUsername, action }: { chats: Chat[]; botUsername: string | null; action: (fd: FormData) => void | Promise<void> }) {
  ...
  return (
    <>
      <button type="button" className="btn pri" onClick={() => ref.current?.showModal()}><Plus size={15} /> Підключити канал або групу{chats.length ? <span className="cnt" style={{ marginLeft: 8, background: "rgba(255,255,255,.25)", color: "#fff" }}>{chats.length}</span> : null}</button>
      <dialog ref={ref} className="modal" style={{ width: "min(600px, 94vw)" }} onClick={(e) => { if (e.target === ref.current) close(); }}>
        <div className="modal-in">
          <div className="modal-h"><h3>{step === 1 ? "Підключити канал або групу" : kind === "channel" ? "Підключити канал" : "Підключити групу"}</h3><button type="button" className="btn sm ghost" onClick={close} aria-label="Закрити"><X size={15} /></button></div>
          {step === 1 && <>
            <div className="opts">
              <button type="button" className={`opt ${kind === "channel" ? "on" : ""}`} onClick={() => setKind("channel")}><span className="steptype"><Megaphone size={16} /></span><b>Канал</b><small>закритий канал із постами</small></button>
              <button type="button" className={`opt ${kind === "group" ? "on" : ""}`} onClick={() => setKind("group")}><span className="steptype"><MessagesSquare size={16} /></span><b>Група</b><small>чат, де пишуть учасниці</small></button>
            </div>
          ...
          {step === 2 && <>
            ...
            <div className="chatlist">
              {list.map((c) => <form key={c.id} action={action} className="chatrow"><input type="hidden" name="chatId" value={c.id} /><span className="steptype">{c.type === "channel" ? <Megaphone size={14} /> : <MessagesSquare size={14} />}</span><span className="grow"><b>{c.title}</b><small className="mono muted">{c.type} · {c.id}</small></span><button type="submit" className="btn sm pri">Підключити</button></form>)}
            ...
            {!manual ? <button type="button" className="lnk" style={{ fontSize: 13, marginTop: 8 }} onClick={() => setManual(true)}>Ввести chat_id вручну</button> : (
              <form action={action} className="chatrow" style={{ marginTop: 8 }}><input name="chatId" placeholder="-1001234567890" className="input grow" required /><button type="submit" className="btn sm">Підключити</button></form>
            )}
