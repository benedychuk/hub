      ...
      <form action={saveStep} className="editor">
        ...
        <div className="form">
          ...
          <Section title="Кроки" description={`${steps.length} у воронці`} className="steps-side">
            {steps.map((s, i) => <Link key={s.id} href={`${base}/${f.id}/steps/${s.id}`} className={`${s.id === step.id ? "on" : ""} ${s.isActive ? "" : "off"}`}><StepIcon type={s.type} /><span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{i + 1}. {s.title || s.type}</span><small>{sendTimeLabel((s.config ?? {}) as StepConfig)}</small></Link>)}
          </Section>
        </div>
      </form>
