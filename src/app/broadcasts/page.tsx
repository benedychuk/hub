    ...
    <Shell title="Розсилки" counts={counts}>
      ...
      <div className="grid g12">
        <div className="card tbl"><h3>Історія</h3><table><thead><tr><th>Назва</th><th>Аудиторія</th><th className="num">Надіслано</th><th className="num">Помилок</th><th>Стан</th><th></th></tr></thead><tbody>
          {list.map((b) => <tr key={b.id}><td>{b.name}<div className="muted" style={{ fontSize: 12 }}>{b.text.slice(0, 70)}</div></td><td className="mono">{(b.audience as { kind?: string }).kind}</td><td className="num">{b.sentCount}</td><td className="num">{b.failedCount}</td><td><Pill tone={b.status === "sent" ? "good" : b.status === "scheduled" ? "moon" : "warn"}>{b.status}{b.scheduledAt ? " · " + dateTime(b.scheduledAt) : ""}</Pill></td>
